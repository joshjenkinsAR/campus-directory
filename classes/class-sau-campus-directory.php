...
class SAU_Campus_Directory {
	...
	function feed_item_enclosure() {
		 ...
		 */
		if ( empty( $url ) )
			$url = esc_url( wp_get_attachment_url( get_post_thumbnail_id() ) );
...
<?php
	...
	function get_archive_entry( $post = null, $i = 0 ) {
		...
		$has_email = str_replace(' ', '', $has_email);
		
		$phone = apply_filters( 'phone-wpcm-value', get_post_meta( get_the_ID(), 'office_phone_wpcm_value', true ) );
		$secondaryphone = apply_filters( 'secondaryphone-wpcm-value', get_post_meta( get_the_ID(), 'secondary_phone_wpcm_value', true ) );
	...
	function get_single_entry() {
		...
		$wpcm_email = str_replace(' ', '', $wpcm_email);
		...
		$wpcm_number_secondary = get_post_meta( $post_ID, 'secondary_phone_wpcm_value', true );
		$wpcm_number_fax = get_post_meta( $post_ID, 'fax_wpcm_value', true );
		$addressone = get_post_meta( $post_ID, 'address1_wpcm_value', true );
		$slotnumber = get_post_meta( $post_ID, 'address_2_wpcm_value', true );
...
if ( ! empty( $terms ) ) {
    foreach( $terms as $term ) {
        $output .= '<a href="' . esc_url( get_term_link( $term->slug, 'department' ) ) . '" class="cat-item-' . $term->term_id . '">' . esc_html( $term->name ) . '</a>';
    }
}
...
<div class="vitals">
	<div class="photo">
    	<img src="' . $wpcm_image_path . '" alt="' . esc_attr( apply_filters( 'the_title', $post->post_title, $post ) ) . '" />';
		 if ( has_term('allears', 'contact-tag') ) { 
		$rt .= '
		<div class="allears-badge"><a href="/title-ix"><img src="https://web.saumag.edu/directory/files/2015/07/AllEars-small.png" /></a></div>';
		}
        $rt .= '
    </div>
    <div id="contact-info">
        ...
        <p class="organization organization-unit">' . trim( $output, $separator ) . '';
		$rt .='</p><div class="info-details">'; 
		...
		$rt .= empty( $wpcm_number_office ) ? '' : '
				<li class="number value" style="clear: both;"><a href="tel:+1-'. $this->format_phone( $wpcm_number_office ) . '">' . $this->format_phone( $wpcm_number_office ) . '</a></span> <span class="type">' . __( '(Office)' ) . '</li>';
				$rt .= empty( $wpcm_number_secondary ) ? '' : '
				<li class="number value" style="clear: both;"><a href="tel:+1-'. $this->format_phone( $wpcm_number_secondary ) . '">' . $this->format_phone( $wpcm_number_secondary ) . '</a></span> <span class="type">' . __( '(Secondary)' ) . '</li>';
		...
		$rt .= empty( $addressone ) ? '' : '
			<span class="post-office-box">MSC ' . $addressone . '</span><br/>';
			if ( ! empty( $bldgoffice ) ) {
		$rt .= '
			<span class="street-address">' . __( 'Building/Office:' ) . ' ' . $bldgoffice . '</span></br>';}
		if ( ! empty( $slotnumber ) ) {
			$rt .= '
			<span class="slot-number">'.__('Slot Number:'). ' ' .$slotnumber. '</span>'; }
...
add_filter('manage_contact_posts_columns', 'post_columns');
function post_columns($defaults){
    $defaults['title-wpcm-value'] = __('Job Title');
    return $defaults;
}
 
//then you need to render the column
add_action('manage_contact_posts_custom_column', 'posts_custom_columns', 1, 2);
function posts_custom_columns($column_name, $post_id){
    if($column_name === 'title-wpcm-value'){
        echo "" . get_post_meta($post_id,'title_wpcm_value', true);
    }
}